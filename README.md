...
```mermaid
graph TD
    subgraph Layer1 ["UI 表现层 (Compose)"]
        UI_Page["[业务页面及组件]"]
        UI_Nav["(全局导航器) ──> (tianquViewModel / 状态与作用域)"]
        UI_Service["(Service Discovery / 获取跨模块服务)"]

        UI_Page -->|"挂起跳转"| UI_Nav
        UI_Page -->|"获取服务"| UI_Service
    end

    subgraph Layer2 ["核心控制层 (Router Runtime)"]
        RT_Host["RouteHost<br/>(路由栈/动画过渡)"]
        RT_Reg["Service Registry<br/>(解析服务映射)"]
        RT_Life["Lifecycle & Scopes<br/>(状态保存与恢复)"]
    end

    subgraph Layer3 ["编译期生成层 (KSP Processor)"]
        KSP_ALL["[开发者声明注解] ──编译期处理──> [自动生成的路由聚合类]"]
    end

    subgraph Layer4 ["业务模块层 (Feature Modules)"]
        F_A["[业务模块 A]"]
        F_B["[业务模块 B]"]
        F_C["[业务模块 C]"]
    end

    %% Layer 1 -> Layer 2 (向下调用)
    UI_Nav ===>|"向下操作"| RT_Host
    UI_Nav -.->|"绑定"| RT_Life
    UI_Service ===>|"向下调用 (挂起)"| RT_Reg

    %% Layer 2 -> Layer 3 (严格向下画线确保排版，通过文字说明是向上注入)
    RT_Host -.->|"⬆ Auto Inject (自动注入依赖与解耦)"| KSP_ALL
    RT_Reg -.->|"⬆ Auto Inject (自动注入依赖与解耦)"| KSP_ALL

    %% Layer 3 -> Layer 4 (严格向下画线确保排版，通过文字说明是向上收集)
    KSP_ALL ===>|"⬆ Auto Aggregate (自动收集与聚合)"| F_A
    KSP_ALL ===>|"⬆ Auto Aggregate (自动收集与聚合)"| F_B
    KSP_ALL ===>|"⬆ Auto Aggregate (自动收集与聚合)"| F_C
```
...
│                              UI 表现层 (Compose)                                 │
│                                                                                 │
│   [业务页面及组件] -----> (全局导航器) -----> (tianquViewModel / 状态与作用域)        │
│         |                      |                                                │
│         |                      | (Suspend Function / 协程挂起)                   │
│         +----------------> (Service Discovery / 获取跨模块服务)                   │
╰─────────────────────────────────────────────────────────────────────────────────╯
                                 |
                                 V
╭─────────────────────────────────────────────────────────────────────────────────╮
│                        核心控制层 (Router Runtime)                                │
│                                                                                 │
│   ╭───────────────────╮    ╭───────────────────╮    ╭───────────────────────╮   │
│   │   RouteHost       │    │  Service Registry │    │ Lifecycle & Scopes    │   │
│   │ (路由栈/动画过渡)    │    │  (解析服务映射)    │    │ (状态保存与恢复)         │   │
│   │ [@Transition]     │    │  [RouterContext]  │    │ (CoroutineScope)      │   │
│   ╰───────────────────╯    ╰───────────────────╯    ╰───────────────────────╯   │
│            ^                        ^                                           │
╰────────────|────────────────────────|───────────────────────────────────────────╯
             |                        |
             +------------------------+ Auto Inject (自动注入依赖与解耦)
             |
╭─────────────────────────────────────────────────────────────────────────────────╮
│                       编译期生成层 (KSP Processor)                                │
│                                                                                 │
│   [开发者声明注解] ----> [编译期符号处理] ----> [自动生成的路由聚合类]                  │
│  (@Router/@Service)      (KSP Processor)        (Generated RouteRegistry)       │
╰─────────────────────────────────────────────────────────────────────────────────╯
             ^
             | Auto Aggregate (自动收集与聚合)
             |
╭─────────────────────────────────────────────────────────────────────────────────╮
